...
public class PlayerMovement : MonoBehaviour {
    ...
    float crouchAnimationSpeed = 3f; // controller height manipulation speed when crouching (default 3)
    ...
    private void Crouch()
    {
        if (Input.GetKey(KeyCode.LeftControl) && !isSprinting)
        {
            ...
            //do a gradual controller height animation and eventually cap it.
            controller.height -= crouchAnimationSpeed * Time.deltaTime;
            if(controller.height <= crouchHeight)
            {
                controller.height = crouchHeight;
            }
        }
        else if (!isHittingCeilingCrouched && !isSprinting && isCrouching)
        {
            //uncrouch

            //these two lines of code make up the animation of standing back up after crouching by simply moving up the controller to counter it expanding to the bottom y side 
            //which would previously make the controller move into the ground. First line moves the controller, Second line changes its height.
            controller.height += crouchAnimationSpeed * Time.deltaTime;
            if(isGrounded) controller.transform.position = new Vector3(controller.transform.position.x, controller.transform.position.y + crouchAnimationSpeed * Time.deltaTime, controller.transform.position.z);
            

            //wait until the controller is back to its orginal size before changing its parameters
            if(controller.height >= originalControllerHeight)
            {
                controller.height = originalControllerHeight;
                isCrouching = false;
                speed = originalControllerSpeed;
